...
const LENGTHS = [
  170, 150, 132, 120, 108, 96, 88, 80, 72, 64,   // old growth
  56, 48, 42, 36, 30, 24, 18, 12, 8, 4,          // a few younger for scale
];

const now = Date.now();
let n = 0;
for (const proj of PROJECTS) {
  proj.openers.slice(0, 5).forEach((opener, i) => {
    const prompts = [opener, ...proj.follow.slice(i % 2), ...proj.follow];
    const turns = LENGTHS[n % LENGTHS.length];
